                ...
                if ($graceMarks > 0) {
                    ...
                    if ($additionalMarks > 0) {
                        ...
                        if ($newGrade > $oldGrade) {
                            $oldGrade=gradmapping($oldGrade);
                            $newGrade=gradmapping($newGrade);
...
function gradmapping($grade) {
    ...
    return $grades[$grade] ?? 0; // Return 0 if subject credits are not found
}


if (isset($_SESSION['messages'])) {
    // Fetch the name from the 'marks' table
    $sql = "SELECT name FROM marks WHERE roll_number = '$rollNumber'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $name = $row['name'];  // Retrieve the value of 'name' from the fetched row

        // Insert the data into the 'log' table
        $allMessages = implode(" ", $_SESSION['messages']);
        $sqlqr = "INSERT INTO `log`(`roll_number`, `name`, `Remarks`) VALUES ('$rollNumber', '$name', '$allMessages')";
        //$result = $conn->query($sqlqr);
    }
}
